...
<body>
				...
				<div class="position-sticky">
					<ul class="nav flex-column">
						...
						<li class="nav-item"><a class="nav-link active"
							onClick="window.location.href='PagInicial.jsp'"> Dashboard </a>
						</li>
						<li class="nav-item"><a class="nav-link"
							href="/les-ecommerce-vinhos/areaAdministrador/Clientes.html">
								Clientes </a></li>
						<li class="nav-item"><a class="nav-link"
								...
								<td>
									<div class="option-button">
										...
										<button type="button" class="btn btn-secondary"
											onClick="confirmarTroca(<%=item.getId()%>, <%=item.getTroca().getId()%> , <%=item.getTroca().getQuantidadeSolicitada()%>,<%=item.getQuantidadeTrocada()%>, <%=pedido.getId()%>, 'EM TROCA', 'TROCA_ACEITA')">Aceitar</button>
										<button type="button" class="btn btn-secondary"
											onClick="confirmarTroca(<%=item.getId()%>, <%=item.getTroca().getId()%> , <%=0%>,<%=item.getQuantidadeTrocada()%>, <%=pedido.getId()%>, 'EM TROCA', 'TROCA_NAO_ACEITA')">Recusar</button>
										<%
										} else if (status == TiposStatusItensPedido.ENVIADO_PARA_TROCA) {
										%>
										<button type="button" class="btn btn-secondary"
											id="confirmarRecebimentoBtn"
											...
											data-novo-status="TROCADO" data-novo-status-item="TROCADO"
											data-bs-toggle="modal"
											data-bs-target="#confirmarRecebimentoModal"
											onclick="capturarDadosConfirmarRecebimento(this)">Confirmar
											Recebimento</button>
										<%
										}
										%>
	...
	<!-- Modal -->
	<div class="modal fade" id="confirmarRecebimentoModal" tabindex="-1"
		aria-labelledby="confirmarRecebimentoModalLabel" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="confirmarRecebimentoModalLabel">Confirmar
						Recebimento</h5>
					<button type="button" class="btn-close" data-bs-dismiss="modal"
						aria-label="Close"></button>
				</div>
				<div class="modal-body text-center">Inserir item no estoque?</div>
				<div class="modal-footer">
					<button type="button" class="btn btn-primary"
						onclick="processarConfirmacao(true)" data-bs-dismiss="modal">Sim</button>
					<button type="button" class="btn btn-secondary"
						onclick="processarConfirmacao(false)" data-bs-dismiss="modal">Não</button>
				</div>
			</div>
		</div>
	</div>
	...
	<script src="../scripts/filtroTabelaTroca.js"></script>
