...
public class SalesDataDao {
	...
	public List<SalesData> fetchSalesData(String productId, String startDate, String endDate) throws SQLException {
		...
		try {
			...
			String query = "SELECT " + "DATE_FORMAT(p.ven_data, '%Y-%m') AS month_year, "
					+ "pi.ped_item_prod_desc AS productName, " + "SUM(pi.ped_item_prod_quantidade) AS volume,"
					+ "SUM(pi.ped_item_prod_valor_total) valorTotal " + "FROM pedido_venda p "
					+ "JOIN pedido_itens pi ON pi.ped_item_ven_id = p.ven_id "
					+ "WHERE (? IS NULL OR pi.ped_item_prod_id = ?) " + "AND p.ven_data BETWEEN ? AND ? "
					+ "GROUP BY 1,2 " + "ORDER BY DATE_FORMAT(p.ven_data, '%Y-%m') ";
