...
public class DaoCarrinho {
	...
	public ArrayList<CarrinhoItens> listarCarrinho(CarrinhoDeCompras carrinho) {
	    ...
	    String read = "SELECT ci.car_itens_id, ci.car_itens_car_id, ci.car_itens_prod_id, ci.car_itens_prod_quant, "
	                + "ci.car_itens_status, ci.car_itens_motivo, p.pro_preco_venda, p.pro_codigo_barra, p.pro_desc "
	                + "FROM carrinho_itens ci "
	                + "INNER JOIN produto p ON ci.car_itens_prod_id = p.pro_id "
	                + "WHERE ci.car_itens_car_id = ? AND ci.car_itens_status <> 2";
	    try {
	        Connection con = Conexao.conectar();
	        PreparedStatement pst = con.prepareStatement(read);
	        ...
	        while (rs.next()) {
	            ...
	            produto.setId(rs.getInt("car_itens_prod_id"));
	            ...
	            carrinho.setId(rs.getInt("car_itens_car_id"));
	            ...
	            item.setId(rs.getInt("car_itens_id"));
	            item.setProduto(produto);
	            item.setQuantProd(rs.getInt("car_itens_prod_quant"));
	            item.setStatus(status); // Atribuindo o status ao objeto CarrinhoItens
	            item.setMotivoRemocao(rs.getString("car_itens_motivo"));
	...
	public String AlterarQuantidade(CarrinhoItens carrinhoItem, int quantidade) {
		String read = "UPDATE  carrinho_itens SET " + "car_itens_prod_quant = ? " + "WHERE car_itens_id = ?";
		try {
			Connection con = Conexao.conectar();
			PreparedStatement pst = con.prepareStatement(read);
	...
	public String alterarStatusCarrinho(CarrinhoItens carrinhoItens) {
		String read = "UPDATE carrinho_itens SET " + "car_itens_status = ?, " + "car_itens_prod_quant = ?, "
				+ "car_itens_motivo = ? " + "WHERE car_itens_car_id = ? " + "and car_itens_prod_id = ? "
				+ "and car_itens_id = ? ";
		try {
			Connection con = Conexao.conectar();
			PreparedStatement pst = con.prepareStatement(read);
			...
			return (carrinhoItens.getStatus() == StatusCarrinhoItens.REMOVIDO)
					? "Item removido do carrinho com sucesso!"
					: (carrinhoItens.getStatus() == StatusCarrinhoItens.ADICIONADO)
							? "Item adicionado do carrinho com sucesso!"
	...
	public ArrayList<CarrinhoItens> listarItens(CarrinhoDeCompras carrinho, int statusItem) {
	    System.out.println("Acesso ao Dao ListarItens, status:"+statusItem);
	    ...
	    String read = "SELECT ci.car_itens_id, ci.car_itens_car_id, ci.car_itens_prod_id, ci.car_itens_prod_quant, "
	                ...
	                + "FROM carrinho_itens ci "
	                + "INNER JOIN produto p ON ci.car_itens_prod_id = p.pro_id "
	                + "WHERE ci.car_itens_car_id = ? AND ci.car_itens_status = ?";
	    try {
	        Connection con = Conexao.conectar();
	        PreparedStatement pst = con.prepareStatement(read);
	        pst.setInt(1, carrinho.getId());
	        pst.setInt(2, statusItem);
	        ResultSet rs = pst.executeQuery();
